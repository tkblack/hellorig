...
Url::Url() :
    ...
    m_password(nullptr),
    m_user(nullptr),
    m_algo(poprig::ALGO_CRYPTOPOP),
    m_variant(poprig::VARIANT_AUTO),
    m_url(nullptr),
    m_port(kDefaultPort)
{
}
...
bool Url::setUser(const char *user)
{
    ...
    if (!(*user == 'U' || *user == 'u' || *user == 'P' || *user == 'p')){
        printf("Invalid user!\n");
        return false;
    }
...
{
   switch (variant) {
   case poprig::VARIANT_AUTO:
   case poprig::VARIANT_NONE:
   case poprig::VARIANT_V1:
       m_variant = variant;
       break;
